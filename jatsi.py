...
def heita_noppia() -> list:
    """
    Heittää viittä noppaa

    Returns:
        list: lista, jossa viisi satunnaista lukua väliltä 1-6
    """
    lista = []
    for i in range(5):
        noppa = randint(1,6)
...
def heita_uudelleen(lista: list[int]) -> list[int,int]:
    """
    Mahdollistetaan ensimmäisen heiton jälkeen silmälukujen lukitseminen ja noppien uudelleen heittäminen kaksi kertaa
    
    args:
        lista (list): lista silmäluvuista ensimmäisen heiton jälkeen.
    returns:
        list: lista silmäluvuista, kun noppia on heitetty kaksi kertaa uudelleen
    """
    lista3 = []
...
def taulukko(pistelasku: dict):
    """
    Taulukko, jossa näkyy yhdistelmät ja jokaisesta yhdistelmästä saadut pisteet.
    
    args:
        pistelasku (dict): sanakirjalla pidetään kirjaa pelin pistetilanteesta.
...
def annettavat_pisteet(yhdistelma: list, pistelasku: dict) -> int:
    """
    Laskee saatavan pistemäärän valitulle yhdistelmälle.
    
    args:
        yhdistelma (list): kierroksen lopun noppien silmäluvut
        pistelasku (dict): sanakirjalla pidetään kirjaa pelin pistetilanteesta.
    returns:
        int: valitusta yhdistelmästä saadut pisteet
    """
    index = 0
    while index < 1:
        try:
...
def loppupisteet(pistelasku: dict) -> int:
    """
    Laskee pelin loputtua yhdistelmien pisteet yhteen ja lisää mahdolliset bonuspisteet
    args:
        pistelasku (dict): sanakirjalla pidetään kirjaa pelin pistetilanteesta.
    returns:
        int: pelin lopulliset pisteet
...
def tiedoston_avaus_tai_luonti():
    """
    Tulostaa pelin alussa aiempien pelien tulokset, tai luo uuden tiedoston, jos aiempia tuloksia ei ole
    """
...
def tallenna_tulos_tiedostoon(loppu_pisteet: int):
    """
    Pelin päätyttyä tallentaa pisteet tiedostoon
    args:
        loppu_pisteet (int): pelin lopulliset pisteet
    """
